...
这里， `<function>` 就是函数名，make支持的函数不多。 `<arguments>` 为函数的参数，参数间以逗号 `,` 分隔，而函数名和参数之间以“空格”分隔。函数调用以 `$` 开头，以圆括号或花括号把函数名和参数括起。函数中的参数可以使用变量，为了风格的统一，函数和变量的括号最好一样，如使用 `$(subst a,b,$(x))` 这样的形式，而不是 `$(subst a,b, ${x})` 的形式。因为统一会更清楚，也会减少一些不必要的麻烦。
...
- `-debug`[=`<options>`]
  ...
  1. a: 也就是all，输出所有的调试信息。（会非常的多）
  2. b: 也就是basic，只输出简单的调试信息。即输出不需要重编译的目标。
  3. v: 也就是verbose，在b选项的级别之上。输出的信息包括哪个makefile被解析，不需要被重编译的依赖文件（或是依赖目标）等。
  4. i: 也就是implicit，输出所有的隐含规则。
  5. j: 也就是jobs，输出执行规则中命令的详细信息，如命令的PID、返回码等。
  6. m: 也就是makefile，输出make读取makefile，更新makefile，执行makefile的信息。
...
# 定义 scripts 文件夹的路径
SCRIPTS_DIR := ../../BSW/Scripts/FR8003

# 默认目标，调用 scripts 文件夹中的 Makefile
all:
	$(MAKE) -C $(SCRIPTS_DIR)

# 清理目标，调用 scripts 文件夹中的 Makefile 的 clean 目标
clean:
...
# ld/lib config                      #
# ---------------------------------- #
LD   := $(PROJECT_DIR)BSW/Scripts/FR8003/ldscript.ld
LD_C := $(PROJECT_DIR)BSW/Scripts/FR8003/syscall_gcc.txt
LIB  := $(PROJECT_DIR)BSW/StartUp/FR8003/libfr800x_stack.a

# ---------------------------------- #
# Source files config                #
# ---------------------------------- #
SRC_FILES += $(PROJECT_DIR)/ASW/Swc/button/button.c
SRC_FILES += $(PROJECT_DIR)/ASW/Swc/led/led.c

SRC_FILES += $(PROJECT_DIR)/ASW/User/src/user_main.c
SRC_FILES += $(PROJECT_DIR)/ASW/User/src/user_task.c
SRC_FILES += $(PROJECT_DIR)/ASW/User/src/user_gap_service.c
SRC_FILES += $(PROJECT_DIR)/ASW/User/src/user_gatt_service.c

SRC_FILES += $(PROJECT_DIR)/BSW/Bsp/FR8003A/src/Bsp_FR8003A_Flash.c
SRC_FILES += $(PROJECT_DIR)/BSW/Bsp/FR8003A/src/Bsp_FR8003A_Gpio.c
SRC_FILES += $(PROJECT_DIR)/BSW/Bsp/FR8003A/src/Bsp_FR8003A_Pmu.c
SRC_FILES += $(PROJECT_DIR)/BSW/Bsp/FR8003A/src/Bsp_FR8003A_Pwm.c

SRC_FILES += $(PROJECT_DIR)/BSW/Drivers/FR8003/src/driver_pwm.c
SRC_FILES += $(PROJECT_DIR)/BSW/Drivers/FR8003/src/driver_flash.c
SRC_FILES += $(PROJECT_DIR)/BSW/Drivers/FR8003/src/driver_gpio.c
SRC_FILES += $(PROJECT_DIR)/BSW/Drivers/FR8003/src/driver_pmu.c
SRC_FILES += $(PROJECT_DIR)/BSW/Drivers/FR8003/src/driver_gpio.c

SRC_FILES += $(PROJECT_DIR)/BSW/StartUp/FR8003/boot_vectors.c

SRC_FILES += $(PROJECT_DIR)/OS/CropFreeOS/co_log.c

# ---------------------------------- #
# Include path config                #
# ---------------------------------- #
INC_PATH += -I"$(PROJECT_DIR)/ASW/SWC/ble"
INC_PATH += -I"$(PROJECT_DIR)/ASW/SWC/button"
INC_PATH += -I"$(PROJECT_DIR)/ASW/SWC/gap"
INC_PATH += -I"$(PROJECT_DIR)/ASW/SWC/gatt"
INC_PATH += -I"$(PROJECT_DIR)/ASW/SWC/led"

INC_PATH += -I"$(PROJECT_DIR)/ASW/Utils"

INC_PATH += -I"$(PROJECT_DIR)/ASW/User/inc"

INC_PATH += -I"$(PROJECT_DIR)/BSW/Bsp/FR8003A/inc"
INC_PATH += -I"$(PROJECT_DIR)/BSW/Cmsis/FR8003"
INC_PATH += -I"$(PROJECT_DIR)/BSW/Drivers/FR8003/inc"

INC_PATH += -I"$(PROJECT_DIR)/OS/CropFreeOS/inc"
...
BASE_ElF  = $(OUTPUT_DIR)/$(PROJECT_NAME).elf
...
$(BASE_ElF) : $(OBJS)
	$(NO_ECHO)$(CC) $(LDFLAGS) -T $(LD) -T $(LD_C) -Wl,-Map,"$(OUTPUT_DIR)/$(PROJECT_NAME).map" -o $(OUTPUT_DIR)/$(PROJECT_NAME).elf $^ $(LIB)
	$(NO_ECHO)$(OBJCOPY) -O binary -S $(OUTPUT_DIR)/$(PROJECT_NAME).elf $(OUTPUT_DIR)/$(PROJECT_NAME).bin
	$(NO_ECHO)$(OBJCOPY) -O ihex   -S $(OUTPUT_DIR)/$(PROJECT_NAME).elf $(OUTPUT_DIR)/$(PROJECT_NAME).hex
	$(NO_ECHO)$(OBJDUMP) -D $(OUTPUT_DIR)/$(PROJECT_NAME).elf > $(OUTPUT_DIR)/$(PROJECT_NAME).dis
	$(info Bulid: creating $(OUTPUT_DIR)/$(PROJECT_NAME).elf)
	$(info Bulid: creating $(OUTPUT_DIR)/$(PROJECT_NAME).map)
	$(info Bulid: creating $(OUTPUT_DIR)/$(PROJECT_NAME).bin)
	$(info Bulid: creating $(OUTPUT_DIR)/$(PROJECT_NAME).hex)
	$(info Bulid: creating $(OUTPUT_DIR)/$(PROJECT_NAME).dis)
...
Target_DONE:
	...
	$(NO_ECHO)$(SIZE) $(OUTPUT_DIR)/$(PROJECT_NAME).elf
...
clean :
	$(NO_ECHO)rm -rf $(OBJECT_DIR) $(OUTPUT_DIR)/*.elf $(OUTPUT_DIR)/*.bin $(OUTPUT_DIR)/*.dis $(OUTPUT_DIR)/*.hex $(OUTPUT_DIR)/*.map
	@echo delete *.o $(OUTPUT_DIR)/*.elf $(OUTPUT_DIR)/*.bin $(OUTPUT_DIR)/*.dis $(OUTPUT_DIR)/*.hex $(OUTPUT_DIR)/*.map
```

...
cover: https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132704192.png
swiper_index: 1
categories: 
  - 嵌入式
tags:
...
[CAN入门书 (eet-china.com)](https://static.assets-stash.eet-china.com/album/old-resources/uploadfile/20110813145308726.pdf)
...
![不同协议的对比](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132704192.png)
...
- 低速CAN使用开环网络，CAN_H和CAN_L其中一端添加2.2kΩ的终端电阻
  ...
  ![CAN硬件电路](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132819193.png)
...
![CAN收发器电路](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132914758.png)
...
![CAN物理层](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125133131660.png)
...
![CAN电平标准](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132958416.png)

高速CAN，总线回归隐性电平快，传输速度就会快，低速CAN总线回归隐性电平慢，传输速度就会慢。

> [!note]
>
> 如果将Can L拉到GND，或将Can H拉到VCC：
>
> 1. 高速Can下，仍然是有差分电平的，终端电阻的存在将会产生压差。
> 2. 低速Can下，将会产生Bus off，因为Can H和Can L将不会产生压差
...
![数据帧](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125131808668.png)
...
![遥控帧](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125131921809.png)
...
1.  位错误（Bit Error）
    1. **产生条件**：节点在发送位（显性0或隐性1）的同时，检测到总线上的实际电平与发送的不一致（仲裁阶段除外）。
    2. **常见原因**：  
       - 总线短路或开路导致电平异常。  
       - 电磁干扰（EMI）造成信号畸变。  
       - 节点硬件故障（如收发器损坏）。
...
![错误帧](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132127740.png)
...
![过载帧](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132200601.png)
...
![帧间隔](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132228586.png)
...
| 段名称                                                 | 段的作用                                                     | Tq数  |
| ------------------------------------------------------ | ------------------------------------------------------------ | ----- |
| 同步段<br> (SS: Synchronization Segment)               | 多个连接在总线上的单元通过此段实现时序调整，同步进行接收和发送的工作。由隐性电平到显性电平的边沿或由显性电平到隐性电平边沿最好出现在此段中。 | 1Tq   |
| 传播时间段 <br>(PTS: Propagation Time Segment)         | 用于吸收网络上的物理延迟的段。 所谓的网络的物理延迟指发送单元的输出延迟、总线上信号的传播延迟、接收单元的输入这个段的时间为以上各延迟时间的和的两倍。 | 1~8Tq |
| 相位缓冲段1 <br>(PBS1: Phase Buffer Segment 1)         | 当信号边沿不能被包含于 SS 段中时，可在此段进行补偿。 由于各单元以各自独立的时钟工作，细微的时钟误差会累积起来，PBS 段可用于吸收此误差，通过对相位缓冲段加减SJW吸收误差。SJW 加大后允许误差加大，但通信速度下降。 | 1~8Tq |
| 相位缓冲段2 <br>(PBS2: Phase Buffer Segment 2)         | ：                                                           | 2~8Tq |
| 再同步补偿宽度 <br>(SJW: reSynchronization Jump Width) | 因时钟频率偏差、传送延迟等，各单元有同步误差。SJW 为补偿此误差的最大值。 | 1~4Tq |

![位时序](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125132558447.png)
...
位时间（单位为秒）计算公式为：
$$
位时间 = \frac{1}{波特率} = 总TQ数 \times TQ时间
$$
TQ数量计算公式为：
$$
总TQ数 = \frac{系统时钟频率}{预分频系数 \times 波特率}
$$
TQ时间计算公式为：
$$
TQ时间 = \frac{1}{系统频率}
...
![硬同步](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125134036907.png)
...
> [!important]
>
> 核心的点是，重同步发生在隐形电平到显现性电平的边沿，此时接收方认为是SS段，那么在往后增加PTS+PSB1的TQ长度处进行采样，所以重同步的目的就是保证能正确的进行采样。

下图是发送方信号沿快于SS，图中的红色部分就是SJW:

![再同步](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125134141237.png)

下图是发送方信号沿慢于SS:

![再同步](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125134226304.png)

由于发送放信号慢于SS时，要在PBS2中减少SJW，**所以要求PBS2必须要大于SJW**。
...
![仲裁过程](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125134418375.png)

## 数据帧和遥控帧的优先级

数据帧和遥控帧ID号一样时，数据帧的优先级高于遥控帧

![数据帧和遥控帧的仲裁过程](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125134503959.png)

## 标准格式和扩展格式的优先级

标准格式的优先级高于扩展格式（SRR必须始终为1，以保证此要求）

![标准格式和拓展格式的仲裁过程](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125134557575.png)
...
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ | --------------------- |
| 位错误     | 比较输出电平和总线电平（不含填充位），当两电平不一样时所检测到的 错误。 | 数据帧（SOF~EOF） <br>遥控帧（SOF~EOF）<br>错误帧<br> 过载帧 | 发送单元 <br>接收单元 |
| 填充错误   | 在需要位填充的段内，连续检测到6 位相同的电平时所检测到的错误。 | 数据帧（SOF~CRC 顺序）<br>遥控帧（SOF~CRC 顺序）             | 发送单元 <br>接收单元 |
| CRC错误    | 从接收到的数据计算出的CRC 结果与接收到的CRC 顺序不同时所检测 到的错误。 | 数据帧（CRC顺序）<br>遥控帧（CRC 顺序）                      | 接收单元              |
| 格式错误   | 检测出与固定格式的位段相反的格式·数据帧时所检测到的错误。    | 数据帧（CRC界定符、ACK界定符、 EOF)<br>遥控帧 （CRC界定符、ACK界定符、 EOF) <br>错误界定符<br>过载界定符 | 接收单元              |
| ACK错误    | 发送单元在ACK槽(ACK Slot)中检测 出隐性电平时所检测到的错误（ACK没被传送过来时所检测到的错误） | 数据帧（ACK槽）<br>遥控帧（ACK槽）                           | 发送单元              |
...
![单元的错误状态](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125135253590.png)
...
| ---- | ------------------------------------------------------------ | -------------------- | ---------------------------------------------------- |
| 1    | 接收单元检测出错误时。<br>例外：接收单元在发送错误标志或过载标志中检测出“位 错误”时，接收错误计数值不增加。 | --                   | +1                                                   |
| 2    | 接收单元在发送完错误标志后检测到的第一个位为显性电平时。     | --                   | +8                                                   |
| 3    | 发送单元在输出错误标志时。                                   | +8                   | --                                                   |
| 4    | 发送单元在发送主动错误标志或过载标志时，检测出位错误。       | +8                   | --                                                   |
| 5    | 接收单元在发送主动错误标志或过载标志时，检测出位错误。       | --                   | +8                                                   |
| 6    | 各单元从主动错误标志、过载标志的最开始检测出连续14 个位的显性位时。 之后，每检测出连续的8个位的显性位时。 | 发送时 +8            | 接收时 +8                                            |
|      | 检测出在被动错误标志后追加的连续8个位的显性位时。            | 发送时 +8            | 接收时 +8                                            |
| 8    | 发送单元正常发送数据结束时（返回 ACK 且到帧结束也未 检测出错误时）。 | -1 <br>TEC=0时±0     | --                                                   |
| 9    | 接收单元正常接收数据结束时(到CRC未检测出错误且正 常返回ACK时）。 | --                   | 1≤REC≤127 时-1 <br>REC=0时±0<br> REC>127时 设REC=127 |
| 10   | 处于总线关闭态的单元，检测到128次连续11个位的隐 性位。       | TEC=0                | REC=0                                                |
...
![网络拓扑结构](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140135890.png)

CAN的控制器在MCU中，而收发器在外部属于外设，CAN控制器将会引出两个引脚，CAN的发送和CAN的接收，CAN的收发器将会将收到的Rx或Tx的数据转换成差分数据，也就是这里的CAN High和CAN Low

CAN收发器原理图：

![收发器原理图](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140217891.png)

对MCU端的接口为TX、RX、VCC和GND，对外接口为CANH和CANL

![收发器框图](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140240108.png)



## 内部设计

![内部设计](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140314915.png)
...
![内部简化框图](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140349986.png)
...
![发送过程](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140421511.png)
...
![接收过程](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140454004.png)
...
![标识符过滤器](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140518675.png)
...
![测试模式](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140605015.png)
...
![工作模式](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140629194.png)
...
![中断](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140739713.png)
...
![时间触发通信](https://cdn.jsdelivr.net/gh/youShouldTrustMe/MyPictures@main/Images/image-20251125140804640.png)

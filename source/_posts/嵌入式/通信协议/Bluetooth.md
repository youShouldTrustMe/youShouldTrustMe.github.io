...
在蓝牙 LE 中，加密过程涉及长期密钥 (LTK)，它是两个连接设备共享和存储的秘密密钥。该密钥在配对过程中生成，并在 AES-CCM 加密算法中用于加密和解密数据。具体来说，在加密过程中，使用 AES-CCM 算法将明文数据与 LTK 相结合，生成密文——加扰的、不可读的数据。只有拥有相应LTK的设备才能解密并访问原始信息。
...
蓝牙的数据签名是一种用于验证数据真实性和完整性的机制，通常通过非对称加密算法实现。其主要功能是确保数据在传输过程中不被篡改，并且来自合法的发送方。其中使用连接签名解析密钥（CSRK）。数据签名可参考[蓝牙数据手册](https://www.mouser.com/pdfdocs/bluetooth-Core-v50.pdf)中的Vol 3 Part C的10.4章节（2077页）。

当设备有数据要发送时，它会使用 CSRK 生成签名并将其附加到数据中。然后，接收设备使用相同的 CSRK 为接收到的数据生成自己的签名。如果生成的签名与附加到数据的签名相匹配，则确认数据的完整性和真实性。经过数据签名的一包数据如下图：

![签名数据的通用格式](https://gitlab.com/18355291538/picture/-/raw/main/pictures/2025/03/6_14_37_30_202503061437465.png)

签名由两部分组成：

1. 消息认证码 (MAC)，由签名算法使用 CSRK 和数据生成
2. 计数器,该计数器就像设备发送签名数据次数的运行计数。它包含在签名中以防止“重放攻击”，即恶意设备拦截数据并尝试重新发送数据。

LE 设备可以发送签名数据，而无需与对等设备建立加密会话。数据应使用 CSRK 进行签名。执行签名验证的设备必须已从签名设备收到 CSRK。发送设备将使用其 CSRK 对传输的数据进行签名。
...
> MAC=CMAC(K,M,Tlen)
> $$
> 其中，CMAC是[NIST SP 800-38B](http://csrc.nist.gov/publications/PubsSPs.html))中的加密算法，其中使用AES-128作为块加密算法。
>
> k是128的密钥CSRK
>
> M是由待签名消息m（数据）和SignCounter（计数器）拼接而成，其中计数器在CSRK生成时初始化为0，每签名一次自增1.
> $$
> M=m||SignCounter
> $$
> Tlen是生成的MAC的长度，一般为64位

当验证时，设备计算接收到的消息和SignCounter的MAC，并与收到的MAC进行对比，如果MAC一致，那么通过验证，反之就失败。

同时，验证设备应该存储最后一个已验证的SignCounter，并与接收到的SignCounter进行比较，如果接收到的SIgnCounter大于存储值，说明消息时最新的，那么更新安全数据库，否则可能时重放攻击。



#### 设备隐私

蓝牙 LE 引入了通过可解析私有地址 (RPA) 来保护设备隐私的措施。这些临时且可变的蓝牙设备地址使得恶意观察者很难根据设备的地址长期跟踪设备，可参考[蓝牙数据手册](https://www.mouser.com/pdfdocs/bluetooth-Core-v50.pdf)的Vol 3 Part C的10.8（2084页）。而蓝牙的随机地址有以下两种：

1. 静态地址

   静态地址是48位随机生成的地址，需要满足以下要求
   ...
   - 地址的随机部分至少有一位应为1
     ...
     ![静态地址的格式](https://gitlab.com/18355291538/picture/-/raw/main/pictures/2025/03/6_16_38_22_202503061638345.png)
...
2. 私有地址
   ...
   1. 不可解析的私有地址
      ...
      ![不可解析地址的格式](https://gitlab.com/18355291538/picture/-/raw/main/pictures/2025/03/6_16_42_0_202503061642894.png)

   2. 可解析的私有地址

      要生成可解析的私有地址，设备必须具有身份解析密钥（IRK，自己或对方的IRK），可解析的私有地址应使用IRK和随机生成的24位数字，随机数称为prand，应满足以下的要求

      - prand的两个最高有效位应等于0和1
      - prand的随机部分至少有一个位应为0
      - prand的随机部分至少有一个位应为1

      ![可解析私有地址格式](https://gitlab.com/18355291538/picture/-/raw/main/pictures/2025/03/6_16_44_18_202503061644195.png)
      ...
      > hash=ah(IRK,prand)
      > $$
      > 最终的随机地址由hash和prand生成的
      > $$
      > PRA=hash||prand
      > $$
      > PRA的高3字节由prand提供，PRA的低3字节由hash提供。
      >
      > 当需要解析的时候，解析方将存储的IRK取出来，并从收到的PRA中提取出prand和hash，此时就可以计算本地的hash
      > $$
      > localHash=ah(IRK,prand)
      > $$
      > 最后将localHash和收到的hash进行对比，如果匹配，则表示对端设备身份已解析
      >
      > 如果设备中具有多个IRK，那么设备将存储的每个IRK重复上述过程，直到最后一个IRK解析完成。
